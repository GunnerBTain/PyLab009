...
#file_path = 'C:/myfolder/abc/thisthing.txt'
#def path_append(file_path: str, repl_str: str) -> str:
    #file_path.replace('.txt', repl_str + '.txt')

def load(file_path, *key_books):
    if os.path.exists(file_path):
        with (open(file_path, 'r') as fp, open(file_path.replace('.json', 'r_.json')) as fp2):
            return json.load(fp2), json.load(fp)
    else:
        process_books(*key_books)
        save(file_path.replace('.json', 'r_.json'), pages)
...
p, cb = load('./code_books/book1.json', 'books/ozymandius.txt')
print(encrypt(cb, 'I am'))
print(decrypt(p, '1-1-0-1-4-62-1-4-36-1-1-122'))

#process_books(('./books/ozymandius.txt'))
